...
//		jfile.setUseSecurity(fileVo.getUseSecurity());
	...
	public JFile getThumbJFile(JFileDetails fileVo, String useSecurity, String uploadPath) {
		if(fileVo == null)
			throw new JFileException(" 첨부파일 이력 정보가 존재하지 않습니다. ");
		...
		JFile jfile = new JFile(getFileDownloadPullPath(fileVo.getFileMask() + ".thumb", fileVo.getUploadMode(), uploadPath));			
		jfile.setOriginalFileName(fileVo.getFileName());
//		jfile.setUseSecurity(fileVo.getUseSecurity());
		jfile.setUseSecurity(useSecurity);
		return jfile;
	...
	public JFile[] getFiles(List<JFileDetails> files, String uploadPath, String useSecurity, String uploadMode) {
		...
		for(JFileDetails file : files) {
			...
			if(logger.isDebugEnabled()) {
				logger.debug("Server Repository Path : "+getFileDownloadPullPath(file.getFileMask(), uploadMode, uploadPath == null ? JProperties.getString(GlobalVariables.DEFAULT_FILE_UPLOAD_PATH_KEY) : uploadPath));
			}
	...
	public void deleteJFiles(JFileDetails details, String uploadPath, String uploadMode) {
		...
		/** thumb파일이 있을경우 삭제한다. */
		deleteFileByFullPath(getFileUploadDirectoryPath(uploadMode, uploadPath)+details.getFileMask() + ".thumb");
	...
	public void upload(MultipartFile multipartFile, JFileDetails fileVo) {
		//TODO: 파일명 변경을 생각해 봐야해.
		//이미지 파일이면 저장경로를 webroot 아래로 둘 수 있도록.
		...
		try {
			JFileUploadTypeFactory.INSTANCE.getUploadType(fileVo.getUseSecurity()).getHandler().handle(multipartFile.getInputStream(), new FileOutputStream(uploadPullPath),uploadPullPath);
