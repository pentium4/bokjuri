...
public class JFileController {
	...
	@RequestMapping("/jfile/jfileImageuploadForm.do")
	public String jfileImageuploadForm() {
		return "egovframework/com/ext/jfile/jfileImageuploadForm";
	}	
	...
	public ModelAndView readFileId(JFileVO fileVO, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			String fileId = service.getFileId(fileVO.getFileId(), request.getParameterValues("fileSeqs[]"));
...
//			}
	...
	@RequestMapping("/jfile/thumbPreview.do")
	public ModelAndView thumbPreview(JFileVO fileVO) {
		JFileDetails prevFileVO = null;
		prevFileVO = service.getAttachFile(fileVO.getFileId(), fileVO.getFileSeq());
		...
		if(prevFileVO == null){
			...
			previewFile = prevFileVO.isImage() ?
					service.getFileThumbBySequence(fileVO.getFileId(), fileVO.getFileSeq(), fileVO.getUseSecurity()) : new JFile(getNoImagePath());
		}
