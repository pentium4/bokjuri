...
require(['init'], function () {
	$(document).ready(function () {
		$("#yearCode").sessionYearCombo({
			 fixVal: "${MM2_25VO_PARAM.yearCode}"	// 고정값
			,top:false
		});
	
		$("#monthCode").monthCombo({
			 fixVal : "${MM2_25VO_PARAM.monthCode}"
			,top:false
		});
		...
		$("#yearCode, #monthCode, #confKindCode").on("change", function(event, strText, strValue){
			...
			params.push({name: "organCode", value: "${MM2_25VO_PARAM.copyOrganCode}"});
		...
		$("#copy").on("click", function(){
			...
			if(!confirm("기존의 총무보고는 모두 삭제되고 복사 됩니다.\n복사 하시겠습니까?")){
				return false;
			}
			...
			$.ajax({
				type: "POST",
				async: false,
				url:  "/MM2/MM2_4/MM2_25/copy.do",
				data: params,
				dataType: 'json',
				success: function(data) {
					if(data.result == "1"){
						$("#organCode", opener.document).val("${MM2_25VO_PARAM.copyOrganCode}");
...
<form:form modelAttribute="MM2_25VO_PARAM">
	<h3><div>총무보고 복사</div></h3>
