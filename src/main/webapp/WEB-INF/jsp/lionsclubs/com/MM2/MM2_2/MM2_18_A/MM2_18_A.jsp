...
<form:form id="form_list" modelAttribute="MM2_18VO">
	...
	<div class="table">
		<table class="tblType1" summary="개인별 납부금 조회표">
			<caption>개인별 납부금 조회</caption>
			<colgroup>
				<col width="100px" />
				<col width="*" />
				<col width="100px" />
	...
	<div class="table">
		...
		<div class="tblTitle Height_0 Scroll_Title widthLong" style="overflow-x: hidden;">
			<table class="tblType2_1" summary="개인별 납부금 조회 안내표">
				<caption>개인별 납부금</caption>
				<colgroup>
					<col width="100px" />
					<c:forEach items="${headList}" var="list1" varStatus="status">
						<col width="180px" />
					</c:forEach>
					<col width="180px" />
				</colgroup>
				<thead>
					<tr>
						<th>성 명</th>
						<c:forEach items="${headList}" var="list2" varStatus="status">
							<th>${list2.code}</th>
						</c:forEach>
						<th>합계</th>
					</tr>
		...
		<div id="Scroll_Area" class="Height_1">
			<table class="tblType2_1 Scroll_Table" id="listTable">
				<colgroup>
					<col width="100px" />
					<c:forEach items="${headList}" var="list3" varStatus="status">
						<col width="180px" />
					</c:forEach>
					<col width="180px" />
				</colgroup>
			...
			<table class="tblType2_1">
				...
				<tfoot>	
					<tr>
						...
						<c:forEach items="${headList}" var="headList" varStatus="status">
							<th class="right">
								<fmt:formatNumber value="${paySum[headList.plusItemCode] }" pattern="#,###"/>
								<c:set var="colTotSum" value="${colTotSum + paySum[headList.plusItemCode] }" />
							</th>
						</c:forEach>
						<th class="right">
