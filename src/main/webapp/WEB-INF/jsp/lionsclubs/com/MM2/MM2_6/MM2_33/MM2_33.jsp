...
require(['init'], function () {
	$(document).ready(function () {
		$("#excelDownload").on("click", function(){
			...
			$(window).unbind();
			$.download("/MM2/MM2_6/MM2_33/excelDownload.do", $("form").serialize());
		});
		...
		$("#sessionYear").sessionYearCombo({
			 top: false
			,fixVal: "${MM2_33VO.sessionYear}"
		});
		
		$("#month").monthCombo({
			 top: false
			,fixVal: "${MM2_33VO.month}"
		});
...
<form:form modelAttribute="MM2_33VO">
		        	...
		        	<c:forEach items="${resultList }" var="list" varStatus="status">
		        		...
		        		<c:forEach items="${list.list }" var="list2" varStatus="status2">
		        			<tr>
		        				...
		        				<td class="right"><fmt:formatNumber value="${list2.amount }" pattern="#,###" /></td>
		        				<td class="center"><fmt:formatDate value="${list2.inexDate }" pattern="yyyy.MM.dd" /></td>
		        				<td title="${lions:newline(list2.inexDesc) }">${list2.inexDesc }</td>
		        			</tr>
							<c:set var="subtotal" value="${subtotal + list2.amount }" />		        			
