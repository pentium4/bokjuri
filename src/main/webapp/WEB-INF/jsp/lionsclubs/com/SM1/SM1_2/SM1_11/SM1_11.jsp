...
require(['init'], function () {
	$(document).ready(function () {
		...
		$("#paramYearCode").sessionYearCombo({
			    fixVal: "${SM1_11VO.paramYearCode}"  // 고정값
			   ,top:true
			   ,topLabel:"전체"
			 });
		...
		$("#retrieveBtn").click(function(){
			$("#form_list").attr("action","/SM1/SM1_2/SM1_11/selectList.do");
			$("#form_list").submit();
		});
		
		$("#initBtn").click(function(){
			$("#form_list").attr("action","/SM1/SM1_2/SM1_11/SM1_11.do");
			$("#form_list").submit();
		});
		
		$("#insertBtn").click(function(){
			if(!validateChecker()) return;
			$("#form_list").attr("action","/SM1/SM1_2/SM1_11/insertList.dox");
			$("#form_list").submit();
		});
		
		$("#updateBtn").click(function(){
			if(!validateChecker()) return;
			$("#form_list").attr("action","/SM1/SM1_2/SM1_11/updateList.dox");
			$("#form_list").submit();
		});
		
		$("#deleteBtn").click(function(){
			$("#form_list").attr("action","/SM1/SM1_2/SM1_11/deleteList.dox");
			$("#form_list").submit();
		});
		...
		$("#reorderBtn").click(function(){
			...
			$("#form_list").attr("action","/SM1/SM1_2/SM1_11/reorderList.dox");
			$("#form_list").submit();
		});
	});
});
...
function validateChecker() {
	...
	<div class="table">
<form:form id="form_list"  modelAttribute="SM1_11VO">
		...
		<tr>
			...
			<th>구분</th>
			<td>
				<lions:LionsSelectTagByList id="paramAppDescCode" name="paramAppDescCode" selValue="${SM1_11VO.paramAppDescCode}" list="${SM1_11VO.codeList}" />
			</td>
		</tr>
		</tbody>
		</table>
	...
	<div class="table">
		...
		<div id="Scroll_Area" class="Height_1">
			<table class="tblType2_1 Scroll_Table">
				...
				<tbody>
<c:forEach items="${SM1_11VO.listData}" var="list" varStatus="status">
					<tr>
						...
						<td>
							<lions:LionsSelectTagByList id="" name="appDescCode" selValue="${list.appDescCode}" list="${SM1_11VO.codeList}" />
						</td>
						...
						<td><input type="text" name="sPos1" class="input" value="<c:out value="${list.sPos1 }" />" maxlength="50" style="width:100%;" /></td>
						<td><input type="text" name="sOrganCode" class="input" value="<c:out value="${list.sOrganCode }" />" maxlength="50" style="width:100%;" /></td>
						<td><input type="text" name="sPos2" class="input" value="<c:out value="${list.sPos2 }" />" maxlength="50" style="width:100%;" /></td>
						<td><input type="text" name="sClubName" class="input" value="<c:out value="${list.sClubName }" />" maxlength="50" style="width:100%;" /></td>
						<td></td>
					</tr>
</c:forEach>
				</tbody>
