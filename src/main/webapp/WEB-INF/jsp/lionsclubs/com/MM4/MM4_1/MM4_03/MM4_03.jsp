...
<div class="table">
	<table class="tblType1" summary="의장,이사 역임자 조회표">
	<caption>의장,이사 역임자 조회</caption>
	<colgroup>
		<col width="100px" />
		<col width="*" />
	</colgroup>
...
<c:forEach items="${LIST_VO.list}" var="list" varStatus="status">
	<c:if test="${status.count % 6 == 1 }"><tr></c:if>
		<td>
			...
			<p class="txt"><c:out value="${list.fAgeCode }" /><br /><c:out value="${list.fOrganCodeText }" /><br /><c:out value="${list.memberName }" /><br /><c:out value="${list.fClubName }" /></p>
		</td>
