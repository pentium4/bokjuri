...
require(['init'], function () {
	$(document).ready(function () {
		...
		$("#yearCode").sessionYearCombo({
			  fixVal: "${MM4_16VO_PARAM.yearCode}"  // 고정값
			  ,top:false
			 });
...
<form:form id="form_list" modelAttribute="MM4_16VO_PARAM">
...
<div class="table">
	<div class="tblTitle Height_0 Scroll_Title widthLong" style="overflow-x: hidden;">
		<table class="tblType2 table-fixed" summary="클럽회장퇴회현황표">	
		...
		<table class="tblType2_1 Scroll_Table" id="listTable">
			...
			<tbody>
				...
				<c:forEach items="${resultList}" var="list" varStatus="status">
					<tr>
						<td>${status.count }</td>	
						<td class="left" title="<c:out value="${list.orgCode }" />" <c:if test="${list.orgCode!='' && MM4_16VO_PARAM.lvl != '3' && MM4_16VO_PARAM.lvl != ''}">onclick="moveSubOrg('${list.orgCode }')" style="cursor:pointer;"</c:if>><c:out value="${list.orgName }" /></td>
