...
require(['init'], function () {
	$(document).ready(function () {
		...
		$("#sessionYear").sessionYearCombo({
			 fixVal: "${MM3_53VO.sessionYear}"	// 고정값
			,top:false
		});
		
		$("#month").monthCombo({
			fixVal : "${MM3_53VO.month}"
		});
...
<form:form id="form_list" modelAttribute="MM3_53VO">
	<input type="hidden" id="memberNo" value="${result.memberNo }" />
	<h3><div><lions:menutitle menuId="${param.menuId }" kind="title" /></div></h3>
	<div class="table">
		<table class="tblType1" summary="클럽별 납부금 조회표">
	...
	<div class="table">
		...
		<div class="tblTitle Height_0 Scroll_Title widthLong" style="overflow-x: hidden;">
			<table class="tblType2_1" summary="클럽별 납부금 조회 안내표">
				<caption>클럽별 납부금</caption>
				<colgroup>
					<col width="100px" />
			...
			<table class="tblType2_1 Scroll_Table" id="listTable">
				...
				<tbody>
					<c:forEach items="${resultList}" var="list" varStatus="status">
						<tr>
							<td class="left" title="${list.clubName }">${list.clubName }</td>
