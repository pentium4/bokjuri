...
require(['init'], function () {
	$(document).ready(function () {
		...
		$("#orgCode").organCode({
			...
			,fixVal:"${MM3_16VO.orgCode}"
		});	
		
		$("#thisYear").sessionYearCombo({
			 fixVal: "${MM3_16VO.thisYear}"  // 고정값
			,top:false
		});
...
<form:form id="form_list" modelAttribute="MM3_16VO">
<h3><div><lions:menutitle menuId="${param.menuId }" kind="title" /></div></h3>
	<div class="table">
		<table class="tblType1" summary="클럽회장퇴회현황표">
			<caption>클럽회장퇴회현황</caption>
			<colgroup>
				<col width="100px" />
				<col width="*" />
			</colgroup>
	...
	<div class="table" style="margin-bottom:15px !important;">
		...
		<div class="tblTitle Height_0 Scroll_Title widthLong" style="overflow-x: hidden;">
			<table class="tblType2_1" summary="클럽회장퇴회현황 조회표">
				<caption>클럽회장퇴회현황</caption>
				<colgroup>
					<col width="50px" />
					<col width="200px" />
				...
				<thead>
					...
					<th class="ellipsis">클럽명</th>
					<th class="ellipsis">조직총회일</th>
					<th class="ellipsis">총원</th>
			...
			<table class="tblType2_1 Scroll_Table" id="listTable">
				...
				<tbody>
					...
					<c:forEach items="${resultList}" var="list" varStatus="status">
						<tr>
								<td>${status.count} </td>
								<td class="left" title="${fn:substring(list.orgCode, 0, 3)}-${fn:substring(list.orgCode, 3, 8)}">${list.orgName}</td>
								<td>
		...
		<div id="ftScroll" class="Scroll_Title Height_0 widthLong" style="margin-bottom:40px; overflow-x: auto;">
