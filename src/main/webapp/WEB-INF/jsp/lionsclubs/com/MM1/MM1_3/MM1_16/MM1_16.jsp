...
require(['init'], function () {
	$(document).ready(function () {
		...
		$("#btnInput").on("click", function(){
			openPopup('<c:url value="/MM1/MM1_3/MM1_16/moveinPopup.do?organCode=${MM1_16VO.organCode}" />', 700, 200, 'moveinPopup');
		});
		...
		*/
		...
		$("#btnDelete").on("click", function(){
			...
			$(".idnNo:checked").each(function(index){
				var idnNo = $(this);
				params.push({name: "moveinMemberArray["+index+"].idnNo", value:idnNo.val()});
			});
			
			$.ajax({
				type: "POST",
				async: false,
				url:  '<c:url value="/MM1/MM1_3/MM1_16/deleteMoveinMember.do" />',
				data: params,
				dataType: 'json',
		...
		$(".approve").on("click", function(){
			...
			$.ajax({
				url:  '<c:url value="/MM1/MM1_3/MM1_16/updateApprove.do" />',
				data: params,
				dataType: 'json',
				success: function(data) {
					if(data.result == "1"){
		...
		$(".approveCancel").on("click", function(){
			...
			$.ajax({
				url:  '<c:url value="/MM1/MM1_3/MM1_16/updateApproveCancel.do" />',
				data: params,
				dataType: 'json',
				success: function(data) {
					if(data.result == "1"){
...
<form:form modelAttribute="MM1_16VO">
	...
	<div class="table">
		<table class="tblType1" summary="퇴회회원 입력">
				...
				<tbody>
					<c:forEach var="list" items="${resultList }" varStatus="status">
						<tr>
							...
							<td class="left" title="${list.preOrganName }">${list.preOrganName }</td>
							<td>
