...
require(['init'], function () {
	$(document).ready(function () {
		...
		$("#organCode").organCode({
			 lvl: 3
			,searchable: true
			,fixVal: "${MM1_16VO.organCode}"
		});
		
		$("#sessionYear").sessionYearCombo({
			 top: false
			,fixVal: "${MM1_16VO.sessionYear}"	// 고정값
		});	
		...
		$(".treatment").on("click", function(){
			...
			var moveinDate = tbl.eq(index).find(".moveinDate");
			
			var params = [];
			params.push({name: "idnNo", value: idnNo.val()});
			params.push({name: "memberNo", value: memberNo.val()});
			params.push({name: "moveinDate", value: moveinDate.val()});
			
			$.ajax({
				url:  '<c:url value="/MM1/MM1_3/MM1_16/updateTreatment.do" />',
				data: params,
				dataType: 'json',
		...
		$(".treatmentCancel").on("click", function(){
			...
			$.ajax({
				url:  '<c:url value="/MM1/MM1_3/MM1_16/updateTreatmentCancel.do" />',
				data: params,
				dataType: 'json',
				success: function(data) {
					if(data.result == "1"){
...
<form:form modelAttribute="MM1_16VO">
	...
	<div class="table">
		<h4>
			...
			<span style="color:#009cff;margin-left: 20px">[회원 전입처리]</span>
		</h4>
		...
		<div class="btnBox">
			<span class="mr20">
				...
				<a href="#" onclick="goUrlSubmit('/MM1/MM1_3/MM1_16/listMoveinMemberTreatment.do')" style=color:#009cff;" class="ml20">
					<strong>
					...
					<c:forEach var="list" items="${resultList }" varStatus="status">
						<tr>
							<td>
								...
								<input type="hidden" name="moveinDate" class="moveinDate" value="${list.moveinDate }" />
							</td>
							...
							<td>
								<fmt:parseDate var="moveinDate" value="${list.moveinDate }" pattern="yyyyMMdd" />
								<fmt:formatDate value="${moveinDate }" pattern="yyyy.MM.dd" />
							</td>
