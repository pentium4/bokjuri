...
require(['init'], function () {
	$(document).ready(function () {
		...
		$("#organCode").organCode({
			 lvl:3
			,searchable: true
			,fixVal: "${MM1_17VO.organCode}"
		});
		
		$("#sessionYear").sessionYearCombo({
			 top: false
			,fixVal: "${MM1_17VO.sessionYear}"	// 고정값
		});		
		...
		$(".treatment").on("click", function(){
			...
			var reinductDate = tbl.eq(index).find(".reinductDate");
			
			var params = [];
			params.push({name: "idnNo", value: idnNo.val()});
			params.push({name: "memberNo", value: memberNo.val()});
			params.push({name: "reinductDate", value: reinductDate.val()});
			
			$.ajax({
				url:  '<c:url value="/MM1/MM1_3/MM1_17/updateTreatment.do" />',
				data: params,
				dataType: 'json',
		...
		$(".treatmentCancel").on("click", function(){
			...
			$.ajax({
				url:  '<c:url value="/MM1/MM1_3/MM1_17/updateTreatmentCancel.do" />',
				data: params,
				dataType: 'json',
				success: function(data) {
					if(data.result == "1"){
...
<form:form modelAttribute="MM1_17VO">
	...
	<div class="table">
		<h4>
			...
			<span style="color:#009cff;margin-left: 20px">[퇴회회원 재입처리]</span>
		</h4>
		...
		<div class="btnBox">
			<span class="mr20">
				...
				<a href="#" onclick="goUrlSubmit('/MM1/MM1_3/MM1_17/listReinductMemberTreatment.do')" style=color:#009cff;" class="ml20">
					<strong>
					...
					<c:forEach var="list" items="${resultList }" varStatus="status">
						<tr>
							<td>
								...
								<input type="hidden" name="reinductDate" class="reinductDate" value="${list.reinductDate }" />
							</td>
							...
							<td>
								<fmt:parseDate var="reinductDate" value="${list.reinductDate }" pattern="yyyyMMdd" />
								<fmt:formatDate value="${reinductDate }" pattern="yyyy.MM.dd" />
							</td>
