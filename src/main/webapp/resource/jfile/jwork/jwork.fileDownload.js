...
jwork.fileDownload.prototype.appendFileRow = function(file) {
	...
	var spanFileName = jQuery("<span>" + file.name + "(" + fileSizeStr + ") - 다운로드수 : " + file.count + "</span>");
	var btnDownload = this.getBtnDownload(file);
	
	var fileAreaTableTr =	jQuery("<li class='type-image tx-existed fileItem' id='" + file.id + "'></li>").append(
							jQuery("<dl></dl>").append(
...
jwork.fileDownload.prototype.changePreview = function(fileId, seq) {
	...
	jQuery(img).prop("src", this.ctx + "/jfile/preview.do?fileId=" + fileId + "&fileSeq=" + seq + "&beanId="+this.beanId + "&uploadPathKey="+this.uploadPathKey);
}
